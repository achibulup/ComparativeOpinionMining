...
class BaseEvaluation(object):
    def __init__(self, config, elems=None, ids_to_tags=None, fold=0, save_model=False):
        """
        :param config: program config table.
        :param elems: ["subject", "object", "aspect", "scale", "predicate"].
        ...
        if ids_to_tags is None:
            self.ids_to_tags = shared_utils.invert_dict(
                shared_utils.create_tag_mapping_ids(elems, config.position_sys, other_flag=True)
            )
        else:
    ...
    ####################################################################################################################
    def get_elem_representation(self, s_index, e_index, polarity):
        """
        :param s_index:
        :param e_index:
        :param polarity:
        ...
        emotion_ids = self.config.val.polarity_dict[polarity]
    ...
    def get_elem_dict(self, target):
        ...
        for i in range(len(result_label_ids)):
            ...
            elems.append(seq_elem)

        assert len(elems) == len(elem_label_ids), "label length error!"

        elem_key = ["subject", "object", "aspect", "predicate"]

        for i in range(len(elems)):
            for j in range(len(elem_label_ids[i])):
                elems[i] = self.sequence_label_convert_dict(
                    elem_label_ids[i][j], elems[i], elem_key[j]
                )
    ...
    def get_ground_truth_num(self, data_dict):
        ...
        :return:
        """
        gold_num = {key: 0 for key in self.config.val.elems}

        for index in range(len(data_dict)):
            for elem in self.config.val.elems:
    ...
    def get_f_score(self, gold_num, predict_num, correct_num, multi_elem_score=False):
        ...
        base_elems = ["subject", "object", "aspect", "predicate"]

        result_dict = self.get_macro_measure(result_dict, base_elems, elem_name="macro")
        result_dict = self.get_micro_measure(
            result_dict, gold_num, predict_num, correct_num, base_elems, elem_name="micro"
        )
    ...
    def get_macro_measure(self, result_dict, multi_key_col, elem_name="macro"):
        ...
        macro_precision = macro_precision / float(len(self.elems))
        macro_recall = macro_recall / float(len(self.elems))
    ...
    def init_elem(self, chosen_elems=None):
        if chosen_elems is None:
            return {elem: 0.0 for elem in self.elems} if self.elems is not None else {}
        return {elem: 0.0 for elem in chosen_elems}
    ...
    def elem_dict_to_string(self, token_list, data_dict):
        ...
        cur_elems = ["subject", "object", "aspect", "predicate"]
        for index, elem in enumerate(cur_elems):
            elem_str += "["
            for elem_index, elem_representation in enumerate(data_dict[elem]):
                ...
                if polarity is not None:
                    elem_str += "..."
                    elem_str += self.config.val.polarities[int(polarity) + 1]

                elem_str += " , " if elem_index != len(data_dict[elem]) - 1 else "]"

            elem_str += "]" if len(data_dict[elem]) == 0 else ""
            elem_str += "; " if index != len(self.elems) - 1 else ""
...
class ElementEvaluation(BaseEvaluation):
    def __init__(self, config, target=None, attn_mask=None, elems=None, ids_to_tags=None, fold=0,
                 save_model=False, comparative_identity=False, gold_sent_label=None):
        super(ElementEvaluation, self).__init__(
            config, elems=elems, ids_to_tags=ids_to_tags, fold=fold, save_model=save_model
        )
    ...
    @staticmethod
    def count_polarity_num(result_elems):
        """
        :param result_elems: [(s_index, e_index, polarity)]
        :return:
        """
        polarity_num = [0] * 4

        for index in range(len(result_elems)):
            polarity_num[result_elems[index][-1] + 1] += 1
...
class PairEvaluation(BaseEvaluation):
    def __init__(self, config, candidate_pair_col, gold_pair_col, elems, ids_to_tags, save_model=False, fold=0):
        super(PairEvaluation, self).__init__(
            config, elems=elems, ids_to_tags=ids_to_tags, save_model=save_model, fold=fold
        )
