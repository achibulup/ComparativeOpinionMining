...
# using elems and position_sys to get {tag: id}
def create_tag_mapping_ids(elems, position_sys, other_flag=True):
    """
    :param elems: like: ["subject", "object", "aspect", "scale", "predicate"]
...
#         if param == "dataset_name":
#             args.dataset_name = value
...
def parameters_to_model_name(param_dict):
    ...
    :param param_dict: {
      "config": {}, 
      "model": model_parameters = {"embed_dropout": args.embed_dropout, "factor": args.factor}}
    :return:
    """
    ...
    # model_name == config1_config2..._confign
    model_name = ""
    ...
    #model_name == config1_config2...confign/param1_param2...paramn
...
def tokenToId(data, convert_dict, dim=1):
    """
    ...
    if dim == 0:
        ...
        for i in range(len(data_ids)):
            data_ids[i] = tokenToId(data_ids[i], convert_dict, dim=dim-1)
...
# make each token array have the same length of max_len
...
def cartesian_product(init_elems, add_elem_list):
    """
    :param init_elems: a list of [(s_index1, e_index1), (s_index2, e_index2)], length is n
    ...
    for index in range(len(init_elems)):
        for add_elem in add_elem_list:
            result_elem_data_col.append(init_elems[index] + [add_elem])

    return result_elem_data_col


# sequence label => elem_dict_v2: {elem: [(s_index, e_index)]}.
def sequence_label_convert_to_elem_dict(sequence_label, elems, position_sys):
    """
    ...
    :return:
    """
    s_index, elem_set = -1, {elem for elem in elems}
    sequence_elem_dict = {elem: [] for elem in elems}
...
def elem_dict_convert_to_multi_sequence_label(token_col: list[list[str]], label_col):
    """
    :param token_col: a list of token list.
    :param label_col: a elem dict like: {elem: {s_index: length}}
    :return:
    """
    elem_pair_col, polarities = [], []
...
def convert_label_dict_by_mapping(label_col, mapping_col):
    ...
    """
    elems = ["subject", "object", "aspect", "scale", "predicate"]
    ...
    for index in range(len(final_label_col)):
        ...
        for pair_index in range(len(final_label_col[index])):
            ...
            init_elem_dict = {k: {} for k in elems}
