...
def read_standard_file(path) -> tuple[(list[str], list[int], list[dict])]:
    """
    :param path:
    :return: ([sentence, ...], [1, 0, ...], [{"subject":...}, ...])
    """
...
def get_sequence_label_item(position_symbol: "B" | "E" | "M", polarity, elem_type, special_symbol: bool):
    """
...
def each_elem_convert_to_multi_sequence_label(
        tokens: list[str], elem_pos: list[tuple], elem_type: "norm" | "predicate", 
        special_symbol: bool) -> (list[str], list[int]):
    """
    :param tokens: tokens in a sentence
    :param elem_pos: [(int, int)...]
    :param elem_type: "norm" or "predicate"
    :param special_symbol:
    :return: (sequence_label : [O,O,...S,M,E,O,O,O,S,M,M,E,O,O....], polarities: [..])
                                      subject      subject
    """
    polarities = []
    sequence_label = ["O"] * len(tokens)

    for elem_position in elem_pos:
        s_index, e_index = elem_position[0], elem_position[1]

        if elem_type == "predicate":
            polarity = elem_position[2]
    ...
    return sequence_label, polarities


def elem_dict_convert_to_multi_sequence_label(token_col: list[list[str]], label_col: list[dict], special_symbol=False):
    """
    ...
    :return:
      elem_pair_col: [
        [[O...SME...]subject, [..]object, [...]aspect], for each sentence
        [],
        ...
        [],
      ]
      result_sequence_label_col: [
        [O...SME...], for each sentence
      ]
      polarities: [[0101...] for each sentence, [...]]
    """
    elem_pair_col, polarities, result_sequence_label_col = [], [], []
    ...
    for index in range(len(token_col)):
        ...
        for elem_index, elem in enumerate(elems):
            ...
            # result may be add special symbol label system.
                ...
                polarities.append(cur_polarity)
                result_sequence_label_col.append(sequence_label)

        elem_pair_col.append(sent_multi_col)

    return elem_pair_col, result_sequence_label_col, polarities
...
def create_polarity_train_data(config, pair_tuple_col, feature_out, bert_feature_out, feature_type=1):
    ...
    :return:
    """
    representation_col, polarities, hidden_size = [], [], 5
    ...
    for index in range(len(pair_tuple_col)):
        for pair_index in range(len(pair_tuple_col[index])):
            ...
            polarities.append([pair_tuple_col[index][pair_index][-1][0] + 1])

    return representation_col, polarities
...
def create_polarity_label(pair_tuple_col):
    ...
    for index in range(len(pair_tuple_col)):
        ...
        polarities.append(sequence_polarity)
